...
namespace luadec.IR {
    ...
    public class Function {
        ...
        /// Any instructions marked as Upval relevant are stored in UpValInstructions list and removed from Instructions list.
        /// </summary>
        public void EliminateRedundantAssignments() {
            for ( int i = 0; i < Instructions.Count(); i++ ) {
                if ( Instructions[i] is Assignment assn && assn.Left.Count() == 1 && !assn.Left[0].HasIndex ) {
                    if ( assn.Right is IdentifierReference reference && !reference.HasIndex ) {
                        ...
                        if ( assn.UpValClosureLevel > -1 ) {
                            if ( UpValInstructions.Count - assn.UpValClosureLevel == 1 ) {
                                UpValInstructions[assn.UpValClosureLevel].Add( assn );
                                forceRemove = true;
                            } else if ( UpValInstructions.Count - assn.UpValClosureLevel == 0 ) {
                                UpValInstructions.Add( new List<Assignment>() );
                                UpValInstructions[assn.UpValClosureLevel].Add( assn );
                                forceRemove = true;
                            } else
        ...
        public void ConstructControlFlowGraph() {
            ...
            for ( int i = 0; i < Instructions.Count(); i++ ) {
                ...
                if ( currentBlock.GetName() == "basicblock_8" ) {
                    foreach ( var h in currentBlock.Instructions ) {
        ...
        public void ConvertToSSA( HashSet<Identifier> allRegisters ) {
            //Console.WriteLine( "Converting {0} to SSA", this.DebugID );
            ...
            Identifier NewName( Identifier orig ) {
                ...
                //Console.Write( orig.Name );
                try {
                    newName.Name = orig.Name + $@"_{Counters[orig]}";
                } catch {
                    Console.WriteLine( "Counters doesn't exist {0}", orig );
                    Console.WriteLine( "{0}, {1}", orig.IType, orig.VType );
                    string.Join( " ", allRegisters ); // to peek it in debug
                    Environment.Exit( -1 );
                }
                //Console.WriteLine( " changed to {0}", newName.Name );
            ...
            void RenameBlock( CFG.BasicBlock b ) {
                ...
                foreach ( var inst in b.Instructions ) {
                    ...
                    foreach ( var def in inst.GetDefines( true ) ) {
                        // we have no business renaming upvals
                        if ( !isMyUpval( def ) )
                ...
                foreach ( var succ in b.DominanceTreeSuccessors ) {
                    if ( succ != EndBlock ) {
                        //Console.WriteLine( "Recursion for block... {0}", succ.GetName() );
                        RenameBlock( succ );
                    }
                }
                ...
                foreach ( var inst in b.Instructions ) {
                    foreach ( var def in inst.GetDefines( true ) ) {
                        // upval won't get renamed and will have this as null
            ...
            foreach ( var b in BlockList ) {
                for ( int i = 0; i < b.Instructions.Count(); i++ ) {
                    ...
                    if ( b.Instructions[i] is Assignment a && a.Right is Closure c ) {
                        //  Console.WriteLine( "UpvalCount = {0}", c.Function.UpvalCount );
                        // Fetch the closure bindings from the list we've saved
                    ...
                    /*
                    for (int j = 1; j <= c.Function.UpvalCount; j++)
                    {

                        if (b.Instructions[i+j] is Assignment ca2) {
                            Console.WriteLine( ca2.Left.Count );
                            Console.WriteLine( ca2.Left[0].Identifier.Regnum );
                            Console.WriteLine( ca2.Right is IdentifierReference );
                            if ( ca2.Right is IdentifierReference ir2 ) {
                                Console.WriteLine( ir2.Identifier.ToString() );
                            }
                        }
                        if (b.Instructions[i + j] is Assignment ca && 
                            ca.Left.Count == 1 && 
                            ca.Left[0].Identifier.Regnum == 0 &&
                            ca.Right is IdentifierReference ir &&
                            ir.Identifier.IType == Identifier.IdentifierType.Register)
                ...
                foreach ( var inst in b.Instructions ) {
                    ...
                    if ( inst is Assignment a && a.Left.Count() > 0 ) {
                        foreach ( var def in a.GetDefines( true ) ) {
                            // don't declare upvals
                            if ( !newdeclared.Contains( def ) && !isMyUpval( def ) ) {
                                newdeclared.Add( def );
                                a.IsLocalDeclaration = true;
                                // Console.WriteLine( "Is a local declaration {0}", a.ToString() );
                ...
                foreach ( var i in b.Instructions ) {
                    if ( i is Assignment a ) {
                        ...
                        foreach ( var l in a.Left ) {
                            // don't rename upvals
                            if ( l is IdentifierReference ir && !ir.HasIndex && ir.Identifier.IType == Identifier.IdentifierType.Register && !renamed.Contains( ir.Identifier ) && !isMyUpval( ir.Identifier ) ) {
                                ...
                                if ( a.LocalAssignments != null && ll < a.LocalAssignments.Count() ) {
                                    ...
                                    //Console.Write( "Renaming variable: {0}", ir.Identifier.Name );
                                    ir.Identifier.Name = $@"f{DebugID}_local{localCounter}";
                                    //Console.WriteLine( " to {0}", ir.Identifier.Name );
                                    localCounter++;
                                }
                            }
                            ll++;
        ...
        bool isMyUpval( Identifier def ) {
            foreach ( var i in this.UpvalueBindings )
                if ( i == def )
                    return true;
            return false;
        }
